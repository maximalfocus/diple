...
# both; a held host is never in that default set. A host that is absent is
# reported apart from one that is present and could not be driven: they are
# different facts, and only the first is a reason to skip.
#
