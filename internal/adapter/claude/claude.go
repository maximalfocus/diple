...
// Align maps each turn onto rows. Turns are matched in order, each from
// the first turn-marker row after the previous turn where its blocks fit;
// a turn that fits nowhere falls back to one paragraph per rendered
// paragraph over the region up to the next prompt marker.
func (a *Adapter) Align(t *adapter.Transcript, rows []string) []adapter.TurnAlignment {
	var out []adapter.TurnAlignment
	pos := 0
	for _, turn := range t.Turns {
		aligned := false
		var spans []align.Span
		firstMarker := -1
		for i := pos; i < len(rows); i++ {
			...
			if s, ok := align.Turn(turn.Blocks, rows, i, rules); ok {
				spans, aligned = s, true
				break
			}
		}
		ta := adapter.TurnAlignment{Turn: turn.Ordinal, Aligned: aligned}
		if aligned {
			last := pos
			for i, b := range turn.Blocks {
				ta.Blocks = append(ta.Blocks, adapter.AlignedBlock{Block: b, Span: adapter.Span{First: spans[i].First, Last: spans[i].Last}})
				if spans[i].Last > last {
					last = spans[i].Last
				}
			}
			pos = last + 1
			...
			to := regionEnd(rows, from+1)
			ta.Blocks = paragraphs(rows, from, to)
			pos = to
		}
		out = append(out, ta)
	}
...
func (a *Adapter) Fallback(rows []string) []adapter.TurnAlignment {
	...
	for i := 0; i < len(rows); i++ {
		...
		out = append(out, adapter.TurnAlignment{Turn: len(out) + 1, Aligned: false, Blocks: paragraphs(rows, i, to)})
		i = to - 1
	}
	return out
}
